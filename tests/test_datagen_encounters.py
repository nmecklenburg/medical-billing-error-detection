...
from claim.datagen.encounters import (
...
from claim.datagen.encounters import matching, parsers
from claim.datagen.encounters.paths import (
    default_extracted_root,
    encounter_cache_metadata_path,
)
...
class DatagenEncounterTests(unittest.TestCase):
    def test_carrier_claim_matches_outpatient_requires_exact_dates_by_default(self) -> None:
        ...
        self.assertTrue(
            matching.carrier_claim_matches_anchor(
                outpatient_anchor,
                date(2008, 3, 14),
                date(2008, 3, 14),
                0,
    ...
    def test_extract_carrier_line_items_pivots_numbered_hcpcs_columns(self) -> None:
        ...
        line_items = parsers.extract_carrier_line_items(row, "196661176988405")
    ...
    def test_merge_facility_anchors_combines_duplicate_claim_rows(self) -> None:
        ...
        merged = matching.merge_facility_anchors(existing, incoming)
    ...
    def test_ensure_encounter_cache_builds_expected_cache_from_fixture_zips(self) -> None:
        with TemporaryDirectory() as tmpdir:
            ...
            metadata = ensure_encounter_cache(
                EncounterBuildConfig(
                    dataset_root=dataset_root,
                    encounter_cache_path=cache_path,
                    sample_number=1,
                    carrier_match_window_days=0,
                    rebuild=False,
                    show_progress=False,
                ),
                CliLogger("test", use_color=False),
            )

            records = list(
                datagen_io.iter_jsonl_records(
                    cache_path,
                    description="records",
                    show_progress=False,
                )
            ...
            self.assertEqual(metadata.clusters_with_carrier, 1)
    ...
    def test_ensure_encounter_cache_reuses_matching_metadata(self) -> None:
        with TemporaryDirectory() as tmpdir:
            dataset_root = Path(tmpdir) / "DE-SynPUF"
            self._write_fixture_dataset(dataset_root)
            cache_path = default_encounter_cache_path(dataset_root, 1, 0)
            ...
            first_metadata = ensure_encounter_cache(config, logger)

            for spec in encounter_constants.SAMPLE_1_FILE_SPECS:
            ...
            second_metadata = ensure_encounter_cache(config, logger)

            self.assertEqual(first_metadata.total_records, second_metadata.total_records)
            self.assertEqual(
                first_metadata.clusters_with_carrier,
                second_metadata.clusters_with_carrier,
            )

    def test_ensure_encounter_cache_rebuilds_stale_metadata(self) -> None:
        with TemporaryDirectory() as tmpdir:
            dataset_root = Path(tmpdir) / "DE-SynPUF"
            self._write_fixture_dataset(dataset_root)
            cache_path = default_encounter_cache_path(dataset_root, 1, 0)
            ...
            logger = CliLogger("test", use_color=False)
            ensure_encounter_cache(config, logger)

            metadata_path = encounter_cache_metadata_path(cache_path)
            with metadata_path.open("w", encoding="utf-8") as handle:
                json.dump(
                    {
                        "schema_version": encounter_constants.ENCOUNTER_CACHE_SCHEMA_VERSION - 1,
                        "sample_number": 1,
                        "carrier_match_window_days": 0,
                    },
                    handle,
            ...
            rebuilt_metadata = ensure_encounter_cache(config, logger)

            self.assertEqual(
                rebuilt_metadata.schema_version,
                encounter_constants.ENCOUNTER_CACHE_SCHEMA_VERSION,
            )

    def test_ensure_encounter_cache_merges_duplicate_outpatient_rows(self) -> None:
        with TemporaryDirectory() as tmpdir:
            dataset_root = Path(tmpdir) / "DE-SynPUF"
            ...
            self.assertTrue(record["summary"]["has_carrier_claim"])
    ...
    def _write_fixture_dataset(
        ...
        *,
        ...
        specs = {spec.filename: spec for spec in encounter_constants.SAMPLE_1_FILE_SPECS}
